--------------------------------------------------------
--  DDL for Package Body PKG_MG_MIGRACION_CARTERA_VAL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GENESYS"."PKG_MG_MIGRACION_CARTERA_VAL" as
	...
	begin
       insert into muerto2(N_001,V_001,T_001) values(1,'Inicio proceso actualiza ID',SYSTIMESTAMP);
       commit;
        DBMS_OUTPUT.PUT_LINE('Inicio prc_mg_actualizar_id_cartera: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));
        ...
        for c_impsto in ( select max(id_intrmdia)                   id_intermdia
							...
							from migra.mg_g_intermedia_val_cartera  z
		...
		loop
            ...
            exception
                when no_data_found then
                    o_cdgo_rspsta  := 1;
                    o_mnsje_rspsta := o_cdgo_rspsta || '. No se encontro el impuesto: ' || c_impsto.cdgo_impsto ||
                                      ' y el subimpuesto: '  || c_impsto.cdgo_impsto_sbmpsto;
                    v_errors.extend;
                    v_errors( v_errors.count ) := t_errors( id_intrmdia => c_impsto.id_intermdia , mnsje_rspsta => o_mnsje_rspsta );
                    --continue;
            ...
            begin
                update migra.mg_g_intermedia_val_cartera z
            ...
            exception
                 when others then
                    o_cdgo_rspsta  := 2;
                    o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar impuesto-subimpuesto.' ||
                                        v_id_impsto || '-' || v_id_impsto_sbmpsto || '.' || sqlerrm;
                    v_errors.extend;
                    v_errors( v_errors.count ) := t_errors( id_intrmdia => c_impsto.id_intermdia , mnsje_rspsta => o_mnsje_rspsta );
                    --continue;
                    DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
       ...
       insert into muerto2(N_001,V_001,T_001) values(1,'Actualiza Impuesto y subimpuesto para registros Identificados',SYSTIMESTAMP);
       commit;
       ...
       begin
           update migra.mg_g_intermedia_val_cartera z
              set z.clmna20 = ( select a.id_cncpto
        ...
        exception
             when others then
                o_cdgo_rspsta  := 3;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar el concepto.' || sqlerrm;
                v_errors.extend;
       ...
       commit;
       insert into muerto2(N_001,V_001,T_001) values(2,'Actualiza el Id Concepto',SYSTIMESTAMP);
       commit;
       DBMS_OUTPUT.PUT_LINE('Actualiza el Id CoLcepto');
        ...
        begin
            update migra.mg_g_intermedia_val_cartera z
               set z.clmna21 = ( select a.id_prdo
                 ...
                 , z.clmna22 = ( select a.cdgo_prdcdad
                                   from df_i_periodos a
                                  where a.cdgo_clnte        = z.cdgo_clnte
                                    and a.id_impsto         = z.clmna18
                                    and a.id_impsto_sbmpsto = z.clmna19
        ...
        exception
             when others then
                o_cdgo_rspsta  := 4;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar el periodo y la periodicidad.' || sqlerrm;
                v_errors.extend;
                v_errors( v_errors.count ) := t_errors( id_intrmdia => null , mnsje_rspsta => o_mnsje_rspsta );
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;
       ...
       commit;
       insert into muerto2(N_001,V_001,T_001) values(3,'Actualiza Periodos y Periocidad',SYSTIMESTAMP);
       commit;
       DBMS_OUTPUT.PUT_LINE('Actualiza Periodos y Periocidad');
        ...
        begin
            update migra.mg_g_intermedia_val_cartera z
               set z.clmna23 = ( select a.id_impsto_acto_cncpto
                ...
                , z.clmna29 = (  select a.gnra_intres_mra
                                   from v_df_i_impuestos_acto_concepto a
                                  where a.id_impsto         = z.clmna18
                                    and a.id_impsto_sbmpsto = z.clmna19
                                    and a.cdgo_impsto_acto  = z.clmna2
        ...
        exception
             when others then
                o_cdgo_rspsta  := 5;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar el impuesto acto concepto. y genera interes de mora' || sqlerrm;
                v_errors.extend;
                v_errors( v_errors.count ) := t_errors( id_intrmdia => null , mnsje_rspsta => o_mnsje_rspsta );
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;
       ...
       commit;
       insert into muerto2(N_001,V_001,T_001) values(4,'Actualiza los Impuestos Acto Conceptos y genera interes',SYSTIMESTAMP);
       commit;
       DBMS_OUTPUT.PUT_LINE('Actualiza los Impuestos Acto Conceptos y genera interes');
        ...
        exception
             when others then
                o_cdgo_rspsta  := 6;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar el impuesto acto concepto del interes.' || sqlerrm;
                v_errors.extend;
                v_errors( v_errors.count ) := t_errors( id_intrmdia => null , mnsje_rspsta => o_mnsje_rspsta );
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;
       ...
       commit;
       insert into muerto2(N_001,V_001,T_001) values(5,'Actualiza Impuesto Acto Concepto de Interes',SYSTIMESTAMP);
       commit;
       DBMS_OUTPUT.PUT_LINE('Actualiza Impuesto Acto Concepto de Interes');
        ...
        exception
             when others then
                o_cdgo_rspsta  := 9;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar la fecha de vencimiento.' || sqlerrm;
                v_errors.extend;
                v_errors( v_errors.count ) := t_errors( id_intrmdia => null , mnsje_rspsta => o_mnsje_rspsta );
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;
       ...
       commit;
       insert into muerto2(N_001,V_001,T_001) values(8,'Actualiza Fecha de Vencimiento',SYSTIMESTAMP);
       commit;
       ...
       begin
          update migra.mg_g_intermedia_val_cartera z
             set z.clmna28 = ( select a.id_cncpto
       ...
       exception
            when others then
                o_cdgo_rspsta  := 10;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible actualizar los conceptos [Grupal].' || sqlerrm;
                v_errors.extend;
                v_errors( v_errors.count ) := t_errors( id_intrmdia => null , mnsje_rspsta => o_mnsje_rspsta );
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;
       ...
       commit;
       insert into muerto2(N_001,V_001,T_001) values(9,'Actualiza Concepto [Grupal] -- clmna23 id Acto concepto',SYSTIMESTAMP);
       commit;
       DBMS_OUTPUT.PUT_LINE('Actualiza Concepto [Grupal] -- clmna23 id Acto concepto');

       commit;
       DBMS_OUTPUT.PUT_LINE('Fin prc_mg_actualizar_id_cartera: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));
    ...
    begin
        ...
        DBMS_OUTPUT.PUT_LINE('Inicio prc_mg_movimiento_financiero: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));

        insert into muerto2(N_001,V_001,T_001) values(1,'Inicio prc_mg_movimiento_financiero',SYSTIMESTAMP);  
        commit;
        ...
        for c_movfinan in (
						  select a.cdgo_clnte
							   ...
							   , a.cdgo_mvmnto_orgn
							   , a.id_orgen
							   , rpad( to_char( a.vgncia || a.id_sjto_impsto ) , 11 , 0 ) as nmro_mvmnto_fncro
							...
							from (
									select z.cdgo_clnte
									  ...
									  from migra.mg_g_intermedia_val_cartera z
                                      join mg_g_sujetos_liquida              l on l.idntfccion = z.clmna1
                                                                              and l.cdgo_lqdcion_estdo = 'A'
									 where z.id_entdad = p_id_entdad 
								  ...
								  group by z.cdgo_clnte
										 ...
										 , decode( z.clmna10 , 'CV' , 'CN' , z.clmna10 )
								  ) a
		...
		loop
            ...
            insert into gf_g_movimientos_financiero (   id_mvmnto_fncro       ,
														cdgo_clnte            ,
														id_impsto             ,
														id_impsto_sbmpsto     ,
														id_sjto_impsto        ,
											...
											values ( sq_gf_g_movimientos_financiero.nextval 
        ...
        insert into muerto2(N_001,V_001,T_001) values(3,'Fin gf_g_movimientos_financiero',SYSTIMESTAMP);  
        commit;

        DBMS_OUTPUT.PUT_LINE('Fin prc_mg_movimiento_financiero: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));
        exception
             when others then
                o_cdgo_rspsta  := 1;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible insertar gf_g_movimientos_financiero.' || sqlerrm;
                v_errors.extend;
                v_errors( v_errors.count ) := t_errors( id_intrmdia => null , mnsje_rspsta => o_mnsje_rspsta );
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;
    ...
    begin
        ...
        insert into muerto2(N_001,V_001,T_001) values(1,'Inicio prc_mg_movimiento_detalle',SYSTIMESTAMP);  
        commit;

        DBMS_OUTPUT.PUT_LINE('Inicio prc_mg_movimiento_detalle: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));
        for c_carteras in (
              select /*+ RESULT_CACHE */
                     ...
                     , case when ( to_number(z.clmna7) - to_number(z.clmna8) ) > 0 then
                          abs( to_number(z.clmna7) - to_number(z.clmna8) )
                       else 0
                       end as vlor_dbe
                     ,  case when ( to_number(z.clmna7) - to_number(z.clmna8) ) < 0 then
                          abs( to_number(z.clmna7) - to_number(z.clmna8) )
            ...
            insert into gf_g_movimientos_detalle ( id_mvmnto_dtlle , id_mvmnto_fncro , cdgo_mvmnto_orgn , id_orgen , cdgo_mvmnto_tpo , vgncia , id_prdo
                                                 ...
                                                 , actvo , gnra_intres_mra , fcha_vncmnto , id_impsto_acto_cncpto, indcdor_mgrdo )
                 values ( sq_gf_g_movimientos_detalle.nextval , c_carteras.id_mvmnto_fncro , c_carteras.cdgo_mvmnto_orgn , c_carteras.id_orgen , c_carteras.cdgo_mvmnto_tpo , c_carteras.vgncia , c_carteras.id_prdo
                        , c_carteras.cdgo_prdcdad , c_carteras.fcha_mvmnto , c_carteras.id_cncpto , c_carteras.id_cncpto_csdo , c_carteras.vlor_dbe , c_carteras.vlor_hber , c_carteras.id_mvmnto_dtlle_bse
                        , c_carteras.actvo , c_carteras.gnra_intres_mra , c_carteras.fcha_vncmnto , c_carteras.id_impsto_acto_cncpto, 'A' );

            update migra.mg_g_intermedia_val_cartera
               set clmna36           = 'OK',
                   cdgo_estdo_rgstro = 'S'
        ...
        insert into muerto2(N_001,V_001,T_001) values(1,'Fin prc_mg_movimiento_detalle',SYSTIMESTAMP);  
        commit;

        DBMS_OUTPUT.PUT_LINE('Fin prc_mg_movimiento_detalle: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));
	...
	begin
        DBMS_OUTPUT.PUT_LINE('Inicio prc_mg_ejecutar_cartera: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));

        /*pkg_mg_migracion_cartera_val.prc_rg_liquidacion_cartera (   p_cdgo_clnte    => p_cdgo_clnte,
                                                                p_id_impsto     => p_id_impsto  );*/

        pkg_mg_migracion_cartera_val.prc_mg_actualizar_id_cartera( p_id_entdad  => p_id_entdad,
        													   p_cdgo_clnte	=> p_cdgo_clnte );

        pkg_mg_migracion_cartera_val.prc_mg_movimiento_financiero( p_id_entdad  => p_id_entdad,
        													   p_cdgo_clnte	=> p_cdgo_clnte );

        pkg_mg_migracion_cartera_val.prc_mg_movimiento_detalle( p_id_entdad => p_id_entdad );

        --pkg_gf_movimientos_financiero.prc_cl_concepto_consolidado();

        commit;

        DBMS_OUTPUT.PUT_LINE('Fin prc_mg_ejecutar_cartera: ' || to_char(sysdate, 'DD/MM/YYYY HH:MI:SS AM'));

        exception
             when others then
                o_cdgo_rspsta  := 1;
                o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible insertar la cartera.' || sqlerrm;
                DBMS_OUTPUT.PUT_LINE(o_mnsje_rspsta);
                return;                
    ...
    as
        --v_id_usrio          sg_g_usuarios.id_usrio%type := 87582;
    ...
    begin
        ...
        insert into muerto2(N_001,V_001,T_001) values(11,'Inicia crear liquidaciones ',SYSTIMESTAMP);
        commit;
        begin
        ...
        exception
          when no_data_found then
            o_cdgo_rspsta  := 1;
            o_mnsje_rspsta := o_cdgo_rspsta ||'. El tipo de liquidaci?n de migraci?n con c?digo [MG], no existe en el sistema.';
            return;
        end;
        ...
        for reg in ( select min(id_intrmdia) as id_intrmdia
                            ...
                            ,json_arrayagg(json_object('id_intrmdia'            value a.id_intrmdia,
                                                       'id_cncpto'              value a.clmna20,
                                                       'id_impsto_acto_cncpto'  value a.clmna23,
                                                       'vlor_lqddo'             value a.clmna7,
                                                       'trfa'                   value 1,
                                                       'bse_cncpto'             value a.clmna7,
                                                       'fcha_vncmnto'           value a.clmna27)
                                           returning clob) as lqdcion_dtlle
                      from migra.mg_g_intermedia_val_cartera a
                      join v_si_i_sujetos_impuesto           c  on c.idntfccion_sjto  = a.clmna1  
        ...
        loop    
            ...
            exception
                when others then
                    rollback;
                    o_cdgo_rspsta  := 2;
                    o_mnsje_rspsta := o_cdgo_rspsta ||'. No fue posible registrar la liquidaci?n.' ||sqlerrm;
                    insert into muerto2(N_001,V_001,T_001) values(11,o_mnsje_rspsta,SYSTIMESTAMP);  
                    commit;
            end;
            ...
            for c_cncptos in (  select a.*
                                  from json_table(reg.lqdcion_dtlle,
                                                  ...
                                                  columns(id_intrmdia           number path '$.id_intrmdia',
                                                          id_cncpto             varchar path '$.id_cncpto',
                                                          ...
                                                          fcha_vncmnto          number path '$.fcha_vncmnto')) a )
            loop            
                    ...
                    values
                      ...
                      update gi_g_liquidaciones
                         set vlor_ttal  = nvl(vlor_ttal, 0) + to_number(c_cncptos.vlor_lqddo),
                             bse_grvble = nvl(bse_grvble, 0) + to_number(c_cncptos.vlor_lqddo) 
                       where id_lqdcion = v_id_lqdcion;

                exception
                    when others then
                        o_cdgo_rspsta  := 3;
                        o_mnsje_rspsta := o_cdgo_rspsta || '. No fue posible crear el registro de liquidaci?n concepto.' ||sqlerrm;
                        rollback;
                        insert into muerto2(N_001,V_001,T_001) values(11,o_mnsje_rspsta,SYSTIMESTAMP);   
                        commit;
                        exit;
            ...
            if ( O_CDGO_RSPSTA = 0 ) then
                insert into mg_g_sujetos_liquida( idntfccion, id_sjto_impsto, id_lqdcion, vgncia, id_prdo, cdgo_lqdcion_estdo )
                                          values( reg.clmna1, reg.id_sjto_impsto, v_id_lqdcion, reg.vgncia, reg.id_prdo, 'A'  );
                commit;
            end if;
            insert into muerto2(N_001,V_001,T_001) values(11,'Insertó mg_g_sujetos_liquida : '||reg.clmna1||'-'||reg.id_sjto_impsto||'-'||reg.vgncia||'-'||reg.id_prdo,SYSTIMESTAMP);
            commit;
        end loop;--  Fin recorre registro de cartera

        insert into muerto2(N_001,V_001,T_001) values(11,'Fin crear liquidaciones PREDIAL',SYSTIMESTAMP);
        commit;

    end prc_rg_liquidacion_cartera;

end pkg_mg_migracion_cartera_val; ---- Fin encabezado del Paquete pkg_mg_migracion_cartera
