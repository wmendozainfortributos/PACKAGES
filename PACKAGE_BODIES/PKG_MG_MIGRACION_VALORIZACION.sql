--------------------------------------------------------
--  DDL for Package Body PKG_MG_MIGRACION_VALORIZACION
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GENESYS"."PKG_MG_MIGRACION_VALORIZACION" as
  ...
  begin
    ...
    select a.*
      ...
      from migra.mg_g_intermedia_val_predios a
     where a.cdgo_clnte = p_cdgo_clnte
       and a.id_entdad = p_id_entdad
       and a.cdgo_estdo_rgstro = 'L'
---       and clmna4 = '0104000002910003000000000'
     order by a.clmna4;
    ...
    insert into muerto2(N_001,V_001,T_001) values(1,'v_id_impsto: '||v_id_impsto,SYSTIMESTAMP); 
    insert into muerto2(N_001,V_001,T_001) values(1,'v_id_impsto_sbmpsto: '||v_id_impsto_sbmpsto,SYSTIMESTAMP); 
    commit;
 ...
 /*   insert into muerto2(N_001,V_001,T_001) values(1,'v_id_sjto: '||v_id_sjto,SYSTIMESTAMP); 
    insert into muerto2(N_001,V_001,T_001) values(1,'v_id_sjto_impsto: '||v_id_sjto_impsto,SYSTIMESTAMP); 
    commit;*/
      ...
      exception
        when no_data_found then
          ...
          exception
            when no_data_found then
              o_cdgo_rspsta  := 9;
              o_mnsje_rspsta := o_cdgo_rspsta ||'. El sujeto estado con codigo #' ||
                                c_prdios.clmna17 ||', no existe en el sistema.';
              v_errors.extend;
              v_errors(v_errors.count) := t_errors(id_intrmdia  => c_prdios.id_intrmdia,
                                                   mnsje_rspsta => o_mnsje_rspsta);
              rollback;
            ...
            when others then
              o_cdgo_rspsta  := 91;
              o_mnsje_rspsta := o_cdgo_rspsta ||'. El sujeto estado con codigo #' ||c_prdios.clmna17 ||
      ...
      begin
        for c_rspnsbles in (select a.id_intrmdia,
          ...
          begin
            ...
            update migra.mg_g_intermedia_val_predios
               set cdgo_estdo_rgstro = 'S'
    ...
    forall j in 1 .. o_ttal_error
      update migra.mg_g_intermedia_val_predios
         set cdgo_estdo_rgstro = 'E', clmna49 = v_errors(j).mnsje_rspsta
       where id_intrmdia = v_errors(j).id_intrmdia;
  ...
  begin
    ...
    select a.clmna4, a.clmna5, a.clmna2, a.clmna3
      bulk collect
      into v_vgncias
      from migra.mg_g_intermedia_val_liquida a
     where a.cdgo_clnte = p_cdgo_clnte
       and a.id_entdad = p_id_entdad
       and a.cdgo_estdo_rgstro = 'L' 
...
*/        
          ...
          begin          
            insert into mg_g_sujetos_liquida(idntfccion,            id_sjto_impsto,           id_lqdcion,    vgncia, id_prdo, CDGO_LQDCION_ESTDO) 
                                      values(c_lqdcnes.idntfccion,  c_lqdcnes.id_sjto_impsto, v_id_lqdcion, 
                                             v_df_i_periodos.vgncia, v_df_i_periodos.id_prdo,   'A');
          exception
            when others then
              o_cdgo_rspsta  := 10;
          ...
          for c_cncptos in ( select id_intrmdia, 
                                    ...
                                    clmna15 as lmta
                               from migra.mg_g_intermedia_val_liq_dtlle
          ...
          --  DBMS_OUTPUT.PUT_LINE('ingresa c_cncptos');
            ...
            update migra.mg_g_intermedia_val_liq_dtlle
               set cdgo_estdo_rgstro = 'S'
             where id_intrmdia = c_cncptos.id_intrmdia;
          end loop; -- Fin Cursor Conceptos  

          if ( o_cdgo_rspsta = 0 ) then   
              update migra.mg_g_intermedia_val_liquida
                 set cdgo_estdo_rgstro = 'S'
...
end pkg_mg_migracion_valorizacion; -- Fin del Paquete
